...
class TestPositionLockMultiProcess(TestBase):
    """
    Test lock server with multiple processes accessing locks simultaneously.

    These tests simulate production usage where:
    - Multiple validator processes run concurrently
    ...
    def test_multiprocess_basic(self):
        """Test that multiple processes can connect and acquire locks"""
        result_queue = Queue()
        race_counter = Value('i', 0)

        miner_hotkey = "5testminer123456789012345678901234567890123456"
        trade_pair_id = "BTCUSD"
        ...
        for i in range(num_workers):
            p = Process(
                ...
                args=(result_queue, i, miner_hotkey, trade_pair_id, 0.01, race_counter)
            )
            processes.append(p)
            p.start()
        ...
        self.assertEqual(success_count, num_workers,
                        f"Expected {num_workers} successes, got {success_count}. Results: {results}")

        # Verify counter was incremented correctly (no race conditions)
        self.assertEqual(race_counter.value, num_workers,
                        f"Race condition detected! Counter is {race_counter.value}, expected {num_workers}")

    def test_multiprocess_concurrent_same_key(self):
        """Test race condition prevention with concurrent access to same key"""
        result_queue = Queue()
        race_counter = Value('i', 0)
        ...
        processes = []
        num_workers = 5  # More workers to increase contention
        ...
        # Verify counter integrity (most important - proves no race condition)
        self.assertEqual(race_counter.value, num_workers,
                        f"Race condition! Counter={race_counter.value}, expected={num_workers}")

    def test_multiprocess_different_keys(self):
        """Test that different keys can be locked independently"""
        result_queue = Queue()
        race_counter = Value('i', 0)

        miner_hotkeys = [
            "5miner_a_12345678901234567890123456789012345",
            "5miner_b_12345678901234567890123456789012345",
            "5miner_c_12345678901234567890123456789012345",
        ]
        trade_pair_id = "BTCUSD"

        processes = []

        # Each process uses a different miner_hotkey
        for i, hotkey in enumerate(miner_hotkeys):
            p = Process(
                target=_multiprocess_worker,
                args=(result_queue, i, hotkey, trade_pair_id, 0.02, race_counter)
            )
            processes.append(p)
        ...
        success_count = sum(1 for r in results if r.startswith("success_"))
        self.assertEqual(success_count, len(miner_hotkeys))

        # Verify counter was incremented (each used different key so no conflict)
        self.assertEqual(race_counter.value, len(miner_hotkeys))


def _counter_worker(result_queue: Queue, worker_id: int, miner_hotkey: str,
                   trade_pair_id: str, iterations: int, shared_counter: Value):
    """
    Worker that performs multiple lock/unlock cycles and increments a shared counter.
    Used to stress test lock correctness.
    """
    try:
        lock_client = PositionLockClient()
