...
> **Deprecated:** This standalone signals server (`run_receive_signals_server.sh`) is deprecated in favor of the REST server that now runs natively from `miner.py` — see [miner_rest_server.md](miner_rest_server.md). Only use this guide if you specifically need to process signals forwarded from TradingView (see [send_signals_from_trading_view_to_ptn.md](send_signals_from_trading_view_to_ptn.md)); for all other integrations, use the miner REST server instead.
