...
Pro accounts run on a parallel bucket track with their own fee schedule, challenge period
rules, and permitted trade pairs. Those values currently mirror the standard account rules
documented above; this section will be updated as the pro rules are finalized.
...
| `account_type` | string | No | `"standard"` (default) or `"pro"` — see [Account Types](#account-types). Set once at creation; immutable afterward. |
