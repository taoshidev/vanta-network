...
**Notes:**
- Raising the tier is allowed at any time. Lowering it requires every position on that subaccount to be closed first. A subaccount created before tiers existed trades tier 0 (each limit is the higher of its old limit and Base); moving it to any tier also requires a flat book.
