...
Equities now include the Russell 1000 index constituents in addition to the curated stocks/ETFs below and a handful of additional single stocks matching Hyperliquid futures offered in the all-market challenge (COIN, CRCL, MSTR, PLTR, SNDK, INTC, HOOD, SPCX) — over 1,000 tickers in total. The full, current list is generated by [`runnable/generate_equity_universe.py`](../runnable/generate_equity_universe.py) and defined in `vali_objects/trade_pair.py`; it's too large to enumerate here in full.
