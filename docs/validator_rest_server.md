...
- `account_type` (string, optional): `"standard"` (default) or `"pro"`. Selects the subaccount's bucket track (`SUBACCOUNT_CHALLENGE`/`SUBACCOUNT_FUNDED` vs `SUBACCOUNT_PRO_CHALLENGE`/`SUBACCOUNT_PRO_FUNDED`) and, with it, its fee schedule, challenge period rules, and permitted trade pairs. Fixed for the life of the subaccount once created. Not part of the signed payload.
