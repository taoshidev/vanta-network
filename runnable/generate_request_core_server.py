...
class RequestCoreManagerServer:
    """
    Server class for request core generation and checkpoint management.

    This class handles the business logic for creating validator checkpoints,
    managing positions data, and uploading to cloud storage. It runs in a separate
    process and uses regular Python dict (no IPC overhead).

    RPC-accessible methods are suffixed with _rpc.
    """

    def __init__(self, position_manager, subtensor_weight_setter, plagiarism_detector, contract_manager=None,
                 asset_selection_manager=None, limit_order_manager=None):
        ...
        # Server runs in separate process, uses regular dict (no IPC needed)
    ...
    def get_compressed_checkpoint_from_memory_rpc(self) -> bytes | None:
        """
        Retrieve compressed validator checkpoint data directly from memory cache (RPC method).

        This is the primary RPC-accessible method for getting the compressed checkpoint.
        """
        try:
    ...
    def health_check_rpc(self) -> bool:
        """Health check method for RPC monitoring."""
        return True
...
if __name__ == "__main__":
    ...
    rcm = RequestCoreManagerServer(position_manager, subtensor_weight_setter, plagiarism_detector, contract_manager=contract_manager, limit_order_manager=limit_order_manager)
