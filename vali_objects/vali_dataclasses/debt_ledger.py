...
class DebtLedgerManager(RPCServiceBase):
    """
    Lightweight RPC client for accessing debt ledger data.

    The actual debt ledger management happens in DebtLedgerManagerServer.
    This client only provides read-only access via RPC.
    ...
    def __init__(self, perf_ledger_manager, position_manager, contract_manager, asset_selection_manager,
        ...
        Args:
            perf_ledger_manager: Performance ledger manager instance (passed to server)
    ...
    def _create_direct_server(self):
        ...
        Returns:
            DebtLedgerManagerServer instance (not proxied, direct Python object)
        """
        from vali_objects.vali_dataclasses.debt_ledger_server import DebtLedgerManagerServer

        return DebtLedgerManagerServer(
            perf_ledger_manager=self._perf_ledger_manager,
    ...
    def _start_server_process(self, address, authkey, server_ready):
        ...
        def server_main():
            ...
            server = DebtLedgerManagerServer(
                perf_ledger_manager=self._perf_ledger_manager,
                position_manager=self._position_manager,
                contract_manager=self._contract_manager,
                asset_selection_manager=self._asset_selection_manager,
