...
class ChallengePeriodManagerServer(CacheController):
    ...
    def refresh(self, current_time: int = None, iteration_epoch=None):
        ...
        # If we can't get eliminations data, skip the entire refresh cycle
        if not eliminations_available:
            return
    ...
    def meets_time_criteria(self, current_time, bucket_start_time, bucket):
        ...
        if bucket == MinerBucket.CHALLENGE:
            bucket_end_time_ms = bucket_start_time + ValiConfig.CHALLENGE_PERIOD_MAXIMUM_MS
            return current_time <= bucket_end_time_ms

        if bucket == MinerBucket.PROBATION:
            bucket_end_time_ms = bucket_start_time + ValiConfig.PROBATION_MAXIMUM_MS
            return current_time <= bucket_end_time_ms
